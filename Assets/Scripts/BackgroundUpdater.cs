...
public class BackgroundUpdater : MonoBehaviour {
	...
	void CheckForRightBorder () {
		bool needToUpdateRightScene = player.transform.position.x > currentScenes[2].transform.position.x;
		if (needToUpdateRightScene) {
			...
			currentScenes[2].transform.Translate(Vector2.right*16.8f*2);
			currentScenes[0].transform.Translate(Vector2.right*16.8f);
		}
	}

	void CheckForLeftBorder () {
		bool needToUpdateLeftScene = player.transform.position.x < currentScenes[0].transform.position.x;
		if (needToUpdateLeftScene) {
			GameObject oldCurrentScene = currentScenes[1];
			currentScenes[1] = currentScenes[0];
			currentScenes[0] = oldCurrentScene;
			currentScenes[0].transform.Translate(-Vector2.right*16.8f*2);
			currentScenes[2].transform.Translate(-Vector2.right*16.8f);
		}
	}
}
